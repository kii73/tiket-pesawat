 ...
 *  dalam  variabel  $mysql  (sesuai  dengan  penggunaan  $mysql->real_escape_string  dan  $mysql->query  pada  kode  asli).
 *  Untuk  Prepared  Statement,  koneksi  harus  berupa  objek  mysqli.
 */
include  "./koneksi.php";

//  Pastikan  user  sudah  login  (username  ada  di  session)
...
if (isset($_GET["pesawat"])) {
    ...
    //  Ambil  input  dan  session,  tidak  perlu  real_escape_string  di  sini  karena  akan  menggunakan  prepared  statement
    $slug  =  $_GET["pesawat"];
    ...
    //  1.  Ambil  data  penerbangan  (pesawat)
    if ($stmt_pesawat  =  $mysql->prepare("SELECT * FROM pesawat WHERE id = ?")) {
        //  's'  menandakan  parameter  adalah  string
        ...
        //  Penanganan  error  untuk  debug  jika  prepared  statement  gagal
    ...
    //  Cek  jika  data  pesawat  tidak  ditemukan
    if (!$pilih) {
        die("Data  penerbangan  tidak  ditemukan  atau  link  tidak  valid.");
    }

    //  Cek  jika  data  user  tidak  ditemukan  (meskipun  sudah  login)
    if (!$user) {
        session_destroy();
        //  Redirect  dengan  informasi  tambahan
...
//  Definisikan  variabel  harga  dan  pajak  untuk  kalkulasi
...
<body>
                                ...
                                <div class="row">
                                    <div class="mb-3  col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="date" class="form-control" disabled id="tanggal" value="<?= date("Y-m-d",  strtotime($pilih["waktu_berangkat"]))  ?>" name="tanggal" required>
                                        </div>
                                    </div>
                                    <div class="mb-3  col-md-6">
                                    ...
                                    <div class="mb-3  col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <select class="form-select" id="kelas" name="kelas" required>
                                                ...
                                                <option <?php echo ($pilih["kelas"]  ??  '')  ==  "First Class"  ?  "selected"  :  ""  ?> value="first class">First Class</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="mb-3  col-md-6">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="tel" class="form-control" id="phone" name="phone" value="<?= htmlspecialchars($user["phone"]  ??  '')  ?>" placeholder="Masukkan  nomor  HP  aktif" required>
    ...
    <script>
        ...
        //  FIX:  Gunakan  variabel  PHP  yang  sudah  didefinisikan  untuk  kalkulasi
        document.getElementById('jumlah').addEventListener('change', function() {
            //  Mengambil  harga  dasar  dan  pajak  dari  PHP
            const basePrice = <?= $base_price  ?>;
            const tax = <?= $tax  ?>;
            const passengers = parseInt(this.value) || 1;

            //  Kalkulasi:  (Harga  Tiket  *  Jumlah  Penumpang)  +  Pajak
